...
char* s21_strerror(int errnum) {
  char* res = s21_NULL;
  char Unknown_error[30];
  if (errnum <= MIN_ERRLIST || errnum >= MAX_ERRLIST) {
    s21_sprintf(Unknown_error, "%s%d", ERROR, errnum);
    res = Unknown_error;
  } else {
  ...
  //   char arr[3] = {'-', abs(errnum) + 48};
  //   res = s21_strncat(Unknown_error, arr, 1);
  // } else {
  //   res = (char*)myerror[errnum];
  // }
  // return res;
  ...
  // } else {
  //   s21_strncpy(result, ((char *)myerror[errnum]), 454);
  // }

  // return result;
  // static char s21_undef[255] = {'\0'};
  // char *res = s21_NULL;
