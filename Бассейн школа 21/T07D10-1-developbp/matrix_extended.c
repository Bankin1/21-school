...
void minmax_stat(int data[][MAX_STOLB], int str, int stolb, int *max, int *min);
void minmax(int **data, int str, int stolb, int *max, int *min);
void output_mas(int *data, int len);
...
int input3() {
    ...
    if (scanf("%d %d", &str, &stolb) == 2 && getchar() == '\n' && str > 0 && stolb > 0) {
        ...
        if (error) {
            int max[str], min[stolb];
            output2(data, str, stolb);
            minmax(data, str, stolb, max, min);
            output_mas(max, str);
            printf("%c", '\n');
            output_mas(min, stolb);
        }
        free(data_val);
        free(data);
    } else
...
int input1(size_t N) {
    ...
    if (error) {
        ...
        minmax_stat(data, str, stolb, max, min);
        output_mas(max, str);
        printf("%c", '\n');
        output_mas(min, stolb);
    }
